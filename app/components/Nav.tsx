    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-sm border-b border-gray-100">
      ...
      {open && (
        <div className="md:hidden border-t border-gray-100 bg-white px-6 py-4 flex flex-col gap-4">
          <Link href="/#work" className="text-sm text-gray-700" onClick={() => setOpen(false)}>Work</Link>
          <Link href="/#about" className="text-sm text-gray-700" onClick={() => setOpen(false)}>About</Link>
          <a href="mailto:rachanaathed572@gmail.com" className="text-sm text-gray-700">Contact</a>
          <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" className="text-sm font-bold" style={{ color: "#3b0764" }}>Resume ↗</a>
        </div>
      )}
    </header>
  );
